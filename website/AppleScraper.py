...
def findAlbumLink(website: str, driver):
    ...
    no_results_elements = driver.find_elements(
        By.XPATH, "//*[text()='No Results' or text()='Try a new search.']"
    )
    if len(no_results_elements) == 2 or len(website.split("/")[-1]) == len(
        "search?term="
    ):
        link = "https://music.apple.com/us/search?term=" + parse.quote(
    ...
    albumlinkElements = driver.find_elements(
        By.XPATH, "(//a[@data-testid='product-lockup-link'])[position()<4]"
    )
...
def GetInfo(driver):
    texts = driver.find_element(
        By.XPATH, "//div[@data-testid='container-detail-header']"
    )
